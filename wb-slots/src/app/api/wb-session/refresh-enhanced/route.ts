...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔄 Refreshing WB session via enhanced system for user ${targetUserId}`);

    // Создаем интеграционный сервис
    const sessionIntegration = new EnhancedSessionIntegration(process.env.ENCRYPTION_KEY);

    try {
      // Обновляем сессию через улучшенную систему
      const result = await sessionIntegration.refreshSession(targetUserId);

      if (result.success) {
        console.log(`✅ Enhanced session refresh successful for user ${targetUserId}`);
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            userId: targetUserId,
            message: 'Session refreshed successfully via enhanced system'
          },
          warnings: result.warnings
        });
      } else {
        console.error(`❌ Enhanced session refresh failed for user ${targetUserId}:`, result.error);
