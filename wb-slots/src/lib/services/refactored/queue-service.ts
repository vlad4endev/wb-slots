...
const logger = new Logger('INFO', { service: 'QueueService' });
...
class NotificationProcessor {
  ...
  private static async sendNotification(channel: TelegramSettings, type: string, data: NotificationData): Promise<void> {
    const config = channel.settings;
  ...
  private static async sendTelegramNotification(config: { botToken?: string; chatId?: string; enabled?: boolean }, type: string, data: NotificationData): Promise<void> {
    try {
  ...
  private static buildTelegramMessage(type: string, data: NotificationData): string {
    switch (type) {
      case 'slot_found':
...
class SlotMonitorProcessor {
  static async processSlotMonitoring(job: Job<MonitorJobData>): Promise<void> {
    ...
    try {
      ...
      const runStatus = foundSlots > 0 ? 'SUCCESS' : 'FAILED';
      
      await DatabaseLogger.updateRunStatus(runId, runStatus, summary);
      
      // Update task status - if successful, mark as completed and disable
...
// ===== HELPER FUNCTIONS =====
export async function addJob(queueName: string, data: Record<string, any>, options?: QueueJobOptions) {
  let queue;
  switch (queueName) {
    case 'slot-search':
