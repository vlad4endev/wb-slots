...
export class RateLimitError extends ExternalServiceError {
  ...
  constructor(public readonly service: string, retryAfter?: number) {
    super(service, `Rate limit exceeded for ${service}`, { retryAfter });
  }
}
...
export class DatabaseError extends AppError {
  ...
  constructor(
    public readonly databaseOperation: string,
    message: string,
    context?: Record<string, any>
  ) {
    super(message, { databaseOperation, ...context });
  }
}
...
export class ErrorFactory {
  ...
  static createDatabaseError(operation: string, message: string, context?: Record<string, any>): DatabaseError {
    return new DatabaseError(operation, message, context);
  }
  ...
  static createBookingError(message: string, bookingCode?: string, slotId?: string, context?: Record<string, any>): BookingError {
    return new BookingError(message, bookingCode, slotId, context);
  }
}
  ...
  static handle(error: unknown): AppError {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Connection')) {
        return ErrorFactory.createDatabaseError('CONNECTION', 'Database connection failed');
      }
...
export class ErrorLogger {
  static log(error: AppError, additionalContext?: Record<string, any>): void {
    const logData = {
      ...error.toJSON(),
      additionalContext,
